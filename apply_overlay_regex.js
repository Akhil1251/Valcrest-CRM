...
const regex = /<div className="mt-4 flex-1 flex flex-col">\s*\{errorMsg && step === 'forgot_password' && \(/;
...
if (regex.test(content)) {
  ...
  const endRegex = /<\/div>\s*<\/form>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*\{\/\* PANE 2: Logo Animation \*\/\}/;
  ...
  if (endRegex.test(content)) {
    content = content.replace(endRegex, endReplace);
